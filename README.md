...
This script will download samples from dbGaP, downsample files, trim off any remaining illumina adapters (adapter sequence file location needs to be specified in "adapt" in global variables), run FastQC (on full file, downsampled file, and trimmed file), rename files, and then run MiXCR. MiXCR will be run on each sample individually (in an array job), and output the results separately for IGK, IGL, and IGH separately. To combine these results and continue processing, see "pipeline_summary_qc.qsub"  
Notes:  
