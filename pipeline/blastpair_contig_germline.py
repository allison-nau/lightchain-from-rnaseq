...
def run_pairs(query_fasta, subject_fasta):
    ...
    # Save filenames:
    if df is not None:
        df["subject_fasta"] = subject_fasta
        df["query_fasta"] = query_fasta
    return df
...
def run_all_files(indir, germdir, outfile):
    ...
    for germline_file in list_of_samples:
        ...
        fasta_file = fasta_file.replace("_germline.fasta", ".fasta.cap.contigs")
        temp_df = run_pairs(query_fasta=fasta_file, subject_fasta=germline_file)
        if temp_df is not None:
            df_list.append(temp_df)
    # Concatenate all pandas dataframes:
    df = pd.concat(df_list, axis=0)
    df.to_csv("blast_temp_intermediate.csv")
    # Create seperate columns for sample, clone id, clone rank, subsequence, etc for both query and subject
    # Query:
    temp_query = df["query_fasta"].str.split(".", expand=True)
    temp_query = temp_query.iloc[:, 0].str.split("/", expand=True)
    temp_query = temp_query.iloc[:, -1].str.split("_", expand=True)
    df["meta_query_sample"] = temp_query.iloc[:, 0]
